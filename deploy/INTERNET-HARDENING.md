...
| Static cache | `curl -I /css/pages/layout_shell.css` shows `Cache-Control` and `ETag` |
...
| Client IP | Peer address by default; `trustproxy 1` trusts only `X-Real-Ip` (loopback bind + `securecookies 1` required). Missing/blank Real-IP → dedicated `proxy-missing-real-ip` key + error log (not peer fallback) |
