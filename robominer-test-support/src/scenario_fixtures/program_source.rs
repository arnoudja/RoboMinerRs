...
pub struct ProgramSourceFixture {
    ...
    pub ai_robot_id: std::cell::Cell<Option<i64>>,
...
impl ProgramSourceFixture {
    pub async fn create(pool: &MySqlPool) -> Self {
        ...
        Self {
            ...
            ai_robot_id: std::cell::Cell::new(None),
    ...
    pub async fn create_linked_robots(&self, pool: &MySqlPool) {
        ...
        let ai_robot_id = crate::insert_ai_robot(
            pool,
            &format!("rust-source-ai-{}", busy_robot_id),
            "rotate(90);",
            1,
        )
        .await;
        ...
        .await;
        self.ai_robot_id.set(Some(ai_robot_id));
    ...
    pub async fn cleanup_rows(&self, pool: &MySqlPool, delete_source: bool) {
        ...
        if let Some(ai_robot_id) = self.ai_robot_id.get() {
