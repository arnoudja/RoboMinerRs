...
pub(crate) async fn claim_achievement_step(
    ...
    match robominer_domain::claim_achievement_step(pool, request)
        .await
...
pub(crate) async fn achievement_states(pool: &robominer_db::MySqlPool, user_id: i64) -> Result<()> {
    let claim_states = robominer_domain::list_achievement_claim_states(pool, user_id)
        .await
        .context("failed to load achievement claim states")?;
    let mined_totals = robominer_domain::list_user_ore_mined_totals(pool, user_id)
        .await
        .context("failed to load user ore mined totals")?;
    let scores = robominer_domain::list_user_best_mining_area_scores(pool, user_id)
        .await
...
pub(crate) async fn achievement_page_states(
    ...
    let robot_count = robominer_domain::count_user_robots(pool, user_id)
        .await
        .context("failed to count user robots")?;
    let achievements = robominer_domain::list_achievement_page_states(pool, user_id)
        .await
        .context("failed to load achievement page states")?;
    let total_requirements =
        robominer_domain::list_achievement_page_total_requirements(pool, user_id)
            .await
            .context("failed to load achievement page total requirements")?;
    let score_requirements =
        robominer_domain::list_achievement_page_score_requirements(pool, user_id)
            .await
