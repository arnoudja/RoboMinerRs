...
pub enum ExecutableExpressionKind {
    ...
    Sqrt(Box<ExecutableExpression>),
    Sin(Box<ExecutableExpression>),
    Cos(Box<ExecutableExpression>),
    Tan(Box<ExecutableExpression>),
...
impl AreaProperty {
    pub fn from_name(name: &str, line: usize) -> Result<Self, CompileError> {
        match name {
            ...
            "miningTurns" | "miningCycles" => Ok(Self::MiningTurns),
