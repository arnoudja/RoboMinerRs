...
pub(crate) fn next_claimable_rally_delay_seconds_with(
    ...
    for area_id in area_ids {
        let area: Vec<&NextClaimRallyCandidate> = candidates
            ...
            .collect();
        let Some(delay) = delay_for_area(&area, rally_size, expiry_start_seconds) else {
            continue;
        };
...
mod tests {
    ...
    fn candidate(area: i64, busy: i32, seconds_left: i32) -> NextClaimRallyCandidate {
    ...
    fn already_claim_ready_via_expiry_yields_zero() {
        let candidates = [candidate(1, 0, 9)];
        assert_eq!(
            next_claimable_rally_delay_seconds_with(&candidates, 4, 10),
            Some(0)
        );
    ...
    fn waits_for_nth_robot_to_free_for_full_rally() {
        ...
        // Fourth robot frees at t=20; only then is a full rally available.
        assert_eq!(
            next_claimable_rally_delay_seconds_with(&candidates, 4, 10),
            Some(20)
        );
    ...
    fn waits_for_expiry_when_partial_queue() {
        let candidates = [candidate(1, 0, 100)];
    ...
    fn expiry_path_waits_for_busy_robot() {
        let candidates = [candidate(1, 50, 20)];
    ...
    fn chooses_sooner_path_across_areas() {
        let candidates = [
            candidate(1, 0, 100), // area 1: expiry in 91s
            candidate(2, 3, 50),  // area 2: one robot free in 3s, then expiry 50-10+1=41 → 41
            candidate(2, 3, 50),
            candidate(2, 3, 50),
            candidate(2, 3, 50), // full rally when free in 3s
        ];
        assert_eq!(
            next_claimable_rally_delay_seconds_with(&candidates, 4, 10),
            Some(3)
